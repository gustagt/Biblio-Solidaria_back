...
class Protocol:
    ...
    def select_last_protocol_id_book(id_book):
        conn = connect()
        retorno_db = conn.execute(
            text(f"SELECT * FROM biblio.protocols WHERE `book_id_book` = {id_book} ORDER BY id_protocol DESC LIMIT 1")
        ).fetchall()
        conn.close()
        return retorno_db
    
    def select_last_protocol():
        conn = connect()
        retorno_db = conn.execute(
            text(f"SELECT * FROM biblio.protocols ORDER BY id_protocol DESC LIMIT 1")
        ).fetchall()
        conn.close()
        return retorno_db